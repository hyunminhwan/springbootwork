...
public class MenuRestController {
  ...
  /* 
   ...
   public Menu findById(@PathVariable(name="id") Long id) {
     ...
     -restFul하게 만들떄는 요구한 자원이 없을 떄 404오류 반환(클라이언트측 오류<없는 id를 넣었기 때문>)
        null일 때 null이 아닐때를 나누어 처리가 필요
     
     * ResponseEntity : 결과 데이터와 HTTP상태코드와 오류코드까지 직접제어할 수 있는 클래스
      -status : 응답에 대한 status 코드
      -header : header 값(요청/응답)에 대한 요구사항
      ...
      * ResponseEntity 사용법
      - status와 body를 이용
        ...
        + 상태코드 HttpStatus에 정의된 값 이용
         ...
         + 상태코드 ok body를 한번에 사용
         ResponseEntity.ok(member)
         
         + body가 없을 때, build() 사용하여 상태코드를 보내줄때
